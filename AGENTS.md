...
- Server needs both entrypoints:
  ...
  if __name__ == "__main__":
      mcp.run()  # Stdio for Codex Desktop / mpak
  ```
...
Codex mcp add folk -- mpak run @nimblebraininc/folk

# Local development (after make pack)
Codex mcp add folk -- mpak run --local /path/to/mcp-folk-0.1.0-darwin-arm64.mcpb
```
